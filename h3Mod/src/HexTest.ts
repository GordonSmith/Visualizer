...
import { BeautifyIcon, Hexagons } from "@hpcc-js/map";
import { extent as d3Extent } from "d3-array";
import { h3ToGeo, h3ToGeoBoundary, polyfill as h3Polyfill } from "h3-js";
import { DivIcon, DomUtil, GridLayer, LatLngBounds, Map, Marker, Point as LPoint, Polygon } from "leaflet";
...
export function tidyBounds(bounds: LatLngBounds) {
...
export class H3Service {
    ...
    query = Query.attach({ baseUrl: "https://play.hpccsystems.com:18002" }, "roxie", "canada");
    query2 = Query.attach({ baseUrl: "http://localhost:8002" }, "roxie", "cities2");

    constructor(readonly childThreshold = 10) {
    }
...
export class HexTest extends Hexagons {
    ...
    private _polygons: { [id: string]: Polygon } = {};
    createH3Indices(data: string[]) {
        ...
        data.forEach(row => {
            ...
            if (!polygon) {
                ...
                polygon = new Polygon(this.fixDateLine(points), {
                    color: "gray",
                    fillColor: "lightgray",
        ...
        data.forEach(row => {
            ...
            if (row.childrows.Row.length) {
                row.childrows.Row.forEach(r => {
                    const marker = new Marker([r.lat, r.lng], {
                        icon: BeautifyIcon({
                            iconShape: "marker",
                            icon: "fa-circle",
                            textColor: "#ffffff",
...
const CanvasLayer = GridLayer.extend({
    createTile(coords, done) {
        ...
        const tile: SVGSVGElement = DomUtil.create("svg", "leaflet-tile") as unknown as SVGSVGElement;
        ...
        service.submit2(coords.z, coords.x, coords.y).then(response => {
            response.forEach(row => {
                ...
                const polygon = DomUtil.create("polygon") as unknown as SVGPolygonElement;
...
export class HexTest2 extends Hexagons {
    ...
    createIcon(childCount) {
        ...
        return new DivIcon({ html: "<div><span>" + childCount + "</span></div>", className: "marker-cluster" + c, iconSize: new LPoint(40, 40) });
    }
    ...
    createH3Indices(data: H3Response) {
        ...
        data.forEach(row => {
            if (row.rowcount < this.service.childThreshold || row.childrows.Row.length) {
                ...
                const circle = new Marker([lat, lon], {
